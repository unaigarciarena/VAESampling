...
try:
    import cma  # cma.fmin is a solver option, "pip install cma" installs cma
except:
    pass
try:
    from scipy.optimize import fmin_slsqp  # "pip install scipy" installs scipy
except:
    pass
...
def coco_optimize(solver, fun, max_evals, max_runs=1e9):
    ...
    for restarts in range(int(max_runs)):
        ...
        if solver.__name__ in ("random_search", ):
            ...
            print(x0)
            solver(fun, x0, iter=1 + remaining_evals / fun.dimension, iprint=-1)
############################ ADD HERE ########################################
